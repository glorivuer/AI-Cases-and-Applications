通过深度分析 xai-org/x-algorithm 代码库（包含 home-mixer 和 phoenix 核心推荐引擎），我为你整理了这份关于 X（原 Twitter）算法机制的详细评估与博主实操指南。
  ...
  收藏即是金：一次“保存”现在相当于50倍的权重加成。如果你的帖子不值得以后参考，它就等于不存在。

75倍规则：回复加上你的回应是算法中最强的信号。忽略评论回复无异于自毁流量。

链接惩罚：外部链接会使流量减少400%。把链接放在个人简介或自动回复里。算法讨厌用户离开平台。

相似集群锁定：系统会根据主题将你分组。偏离你的领域，你的内容分发量就会降到零。

停留时间：如果用户不点击“查看更多”或观看视频，你的排名就会下降。吸引用户停留，否则就会失去曝光。

负面权重：静音和屏蔽比取消关注的杀伤力大10倍。你可以有争议性，但不要发布垃圾信息。
